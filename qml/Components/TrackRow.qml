...
/*
 * Универсальная строка трека.
 *
 * Раньше этот код был скопирован в 8+ местах (плейлисты, недавние,
 * моя волна, поиск, альбом, исполнитель, лайки, радиостанция) с
 * мелкими отличиями. Теперь отличия задаются свойствами, а не
 * копипастой:
 *
 *   - activated()       — левый клик по строке; родитель сам вызывает
 *                         нужный select*() (зависит от источника)
 *   - addRequested()    — клик по "+" (родитель открывает PlaylistPicker)
 *   - removeRequested() — клик по "×" (удаление из плейлиста)
 *   - правый клик       — копирование названия/исполнителя (copyTrack)
 *   - клик по исполнителю/альбому — loadArtist()/loadAlbum()
 */
Item {
    id: root
    ...
    Rectangle {
        ...
        Column {
            ...
            // Исполнитель — кликабельная ссылка на страницу артиста
            Item {
                id: artistArea
            ...
            // Альбом — кликабельная ссылка на страницу альбома
            Item {
                id: albumArea
        ...
        // ---------------------------------------------------------
        // Кнопки справа (удалить / добавить / лайк)
        // ---------------------------------------------------------
        ...
        MouseArea {
            ...
            onPressed: function(mouse) {
                // Правый клик — скопировать название/исполнителя
