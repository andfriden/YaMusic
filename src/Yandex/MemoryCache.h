...
// без мьютекса.
template <typename T> class MemoryCache {
public:
  explicit MemoryCache(qint64 ttlMs = 5 * 60 * 1000) : m_ttlMs(ttlMs) {}
  ...
  bool get(const QString &key, T &value) const {
    ...
    if (QDateTime::currentMSecsSinceEpoch() - it->first > m_ttlMs) {
      m_items.erase(it);
      return false;
    }
  ...
  void put(const QString &key, const T &value) {
    m_items.insert(key, qMakePair(QDateTime::currentMSecsSinceEpoch(), value));
  }

  void remove(const QString &key) { m_items.remove(key); }
