...
void StationService::loadStationTracks(
    ...
    const QString &queueTrackId)
{
    ...
    const QString path =
        QString("/rotor/station/%1:%2/tracks?%3")
            .arg(stationType)
            .arg(stationId)
            .arg(query.toString(
                QUrl::FullyEncoded));
    ...
    if (reply == nullptr) {
        ...
        emit errorOccurred(
            "Не удалось создать запрос станции");
...
void StationService::loadMoreStationTracks(
    const QString &stationType,
    const QString &stationId,
    const QString &queueTrackId)
{
...
void StationService::sendFeedback(
    const QString &stationType,
    const QString &stationId,
    const QString &event,
    const QString &trackId,
    ...
    qint64 totalPlayedSeconds)
{
    ...
    const QString path =
        QString("/rotor/station/%1:%2/feedback?%3")
            .arg(stationType)
            .arg(stationId)
            .arg(query.toString(
                QUrl::FullyEncoded));
...
Track StationService::parseTrack(
