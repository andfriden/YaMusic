...
QHash<int, QByteArray> SimilarArtistsModel::roleNames() const {
  return {{IdRole, "artistId"}, {NameRole, "name"}, {CoverUriRole, "coverUri"}};
}

void SimilarArtistsModel::setArtists(const QList<Artist> &artists) {
  beginResetModel();
  m_artists = artists;
  endResetModel();
}
...
QList<Artist> SimilarArtistsModel::artists() const {
  return m_artists;
}
