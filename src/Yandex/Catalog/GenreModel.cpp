...
QVariant GenreModel::data(const QModelIndex &index, int role) const {
  ...
  const Genre &genre = m_genres.at(row);
  ...
  case SubGenresRole: {
    ...
    for (const Genre &subGenre : genre.subGenres) {
      ...
      item.insert("title", subGenre.title);
      item.insert("fullTitle", subGenre.fullTitle);
      item.insert("urlPart", subGenre.urlPart);
      item.insert("color", subGenre.color);
      item.insert("image208", subGenre.image208);
      item.insert("image300", subGenre.image300);
      item.insert("showInMenu", subGenre.showInMenu);
      result.append(item);
    }
    return result;
  }
...
QHash<int, QByteArray> GenreModel::roleNames() const {
  return {{IdRole, "genreId"},
          {TitleRole, "title"},
          {FullTitleRole, "fullTitle"},
          {UrlPartRole, "urlPart"},
          {ColorRole, "color"},
          {Image208Role, "image208"},
          {Image300Role, "image300"},
          {ShowInMenuRole, "showInMenu"},
          {HasSubGenresRole, "hasSubGenres"},
          {SubGenresRole, "subGenres"}};
}

void GenreModel::setGenres(const QList<Genre> &genres) {
  beginResetModel();
  m_genres = genres;
  endResetModel();
}
...
Genre GenreModel::genreAt(int index) const {
