...
QHash<int, QByteArray> GenreStationModel::roleNames() const {
...
void GenreStationModel::appendTracks(const QList<Track> &tracks) {
  if (tracks.isEmpty()) {
    return;
  }

  const int firstNew = m_tracks.size();
  const int lastNew = firstNew + tracks.size() - 1;
