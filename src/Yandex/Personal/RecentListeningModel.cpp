...
QHash<int, QByteArray> RecentListeningModel::roleNames() const {
...
void RecentListeningModel::setTracks(const QList<Track> &tracks) {
...
QVariantMap RecentListeningModel::trackDataAt(int index) const {
  ...
  result.insert("sourceIndex", index);
  result.insert("trackId", track.id);
  result.insert("title", track.title);
  result.insert("artist", artistName);
  result.insert("artistId", artistId);
  result.insert("album", albumTitle);
  result.insert("albumId", albumId);
...
QVariantList RecentListeningModel::randomTrackData(int limit) const {
  ...
  for (int i = indexes.size() - 1; i > 0; --i) {
    ...
    indexes.swapItemsAt(i, j);
  }

  const int resultCount = qMin(limit, indexes.size());

  for (int i = 0; i < resultCount; ++i) {
    const int sourceIndex = indexes.at(i);
    const Track &track = m_tracks.at(sourceIndex);
    QVariantMap item;
    QString artistName;
    QString artistId;
    ...
    item.insert("sourceIndex", sourceIndex);
    item.insert("trackId", track.id);
    item.insert("title", track.title);
    item.insert("artist", artistName);
    item.insert("artistId", artistId);
    item.insert("album", albumTitle);
    item.insert("albumId", albumId);
    item.insert("coverUri", track.coverUri);
    item.insert("durationMs", track.durationMs);
    result.append(item);
  }
  return result;
}
