...
QHash<int, QByteArray> PersonalChartModel::roleNames() const {
  return {{IdRole, "trackId"},        {PositionRole, "position"}, {ListenersRole, "listeners"},
          {ProgressRole, "progress"}, {ShiftRole, "shift"},       {TitleRole, "title"},
          {ArtistsRole, "artists"},   {CoverUriRole, "coverUri"}, {DurationMsRole, "durationMs"}};
}

void PersonalChartModel::setItems(const QList<PersonalChartItem> &items) {
  beginResetModel();
  m_items = items;
  endResetModel();
}
