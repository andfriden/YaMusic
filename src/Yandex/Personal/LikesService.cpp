...
LikesService::LikesService(YandexAuth *auth, QObject *parent) : YandexServiceBase(auth, parent) {}

void LikesService::loadLikedTracks(const QString &uid) {
  ...
  const QString path = QStringLiteral("/users/%1/likes/tracks").arg(uid.trimmed());
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply]() {
    ...
    if (parseError.error != QJsonParseError::NoError) {
      ...
      m_loadingTracks = false;
      emit loadingChanged(false);
      emit errorOccurred(QStringLiteral("Не удалось разобрать список понравившихся треков"));
      return;
    }
    ...
    if (!document.isObject()) {
      ...
      m_loadingTracks = false;
      emit loadingChanged(false);
      emit errorOccurred(QStringLiteral("Некорректный ответ списка понравившихся треков"));
      return;
    }

    const QJsonObject root = document.object();
    const QJsonObject result = root.value(QStringLiteral("result")).toObject();
    const QJsonObject library = result.value(QStringLiteral("library")).toObject();
    const QJsonArray tracks = library.value(QStringLiteral("tracks")).toArray();
    ...
    for (const QJsonValue &value : tracks) {
      ...
      if (value.isObject()) {
        ...
        if (idValue.isString()) {
          trackId = idValue.toString();
        } else if (idValue.isDouble()) {
          trackId = QString::number(static_cast<qint64>(idValue.toDouble()));
        }
      } else if (value.isString()) {
        trackId = value.toString();
      } else if (value.isDouble()) {
        trackId = QString::number(static_cast<qint64>(value.toDouble()));
      }

      trackId = trackId.trimmed();
      if (trackId.isEmpty()) continue;
      if (uniqueIds.contains(trackId)) continue;
...
void LikesService::loadLikedAlbums(const QString &uid) {
  ...
  emit loadingChanged(true);
  const QString path = QStringLiteral("/users/%1/likes/albums").arg(userId);
  QNetworkReply *reply = m_yandexClient->get(path);
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply]() {
    ...
    for (const QJsonValue &value : albumData) {
      ...
      if (!album.id.isEmpty()) m_likedAlbumIds.insert(album.id);
      ...
      if (!album.id.isEmpty() || !album.title.isEmpty()) albums.append(album);
    }
...
void LikesService::loadLikedArtists(const QString &uid) {
  ...
  emit loadingChanged(true);
  const QString path = QStringLiteral("/users/%1/likes/artists").arg(userId);
  QNetworkReply *reply = m_yandexClient->get(path);
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply]() {
    ...
    for (const QJsonValue &value : artistData) {
      ...
      if (!artist.id.isEmpty()) m_likedArtistIds.insert(artist.id);
      ...
      if (!artist.id.isEmpty() || !artist.name.isEmpty()) artists.append(artist);
    }
...
void LikesService::loadTracksByIds(const QStringList &trackIds) {
  ...
  *tracksConnection = connect(
      m_yandexClient, &YandexClient::tracksReceived, this,
      [this, tracksConnection](const QList<Track> &tracks) {
        disconnect(*tracksConnection);
        QList<Track> likedTracks = tracks;
  ...
  *errorConnection = connect(
      m_yandexClient, &YandexClient::requestError, this,
      [this, errorConnection](const QString &message) {
...
void LikesService::addLike(const QString &uid, const QString &trackId) {
  changeLike(uid, trackId, true);
}

void LikesService::removeLike(const QString &uid, const QString &trackId) {
  changeLike(uid, trackId, false);
}

bool LikesService::isLiked(const QString &trackId) const {
  const QString id = trackId.trimmed();
  if (id.isEmpty()) return false;
  return m_likedTrackIds.contains(id);
}

void LikesService::changeLike(const QString &uid, const QString &trackId, bool liked) {
  if (!ensureAuthenticated()) {
    emit errorOccurred(QStringLiteral("Токен Яндекс Музыки не установлен"));
    return;
  }
  ...
  if (id.isEmpty()) {
    emit errorOccurred(QStringLiteral("ID трека не указан"));
    return;
  }
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply, id, liked]() {
    const QByteArray data = reply->readAll();
    ...
    reply->deleteLater();
    emit likeChanged(id, liked);
  });
}

void LikesService::addAlbumLike(const QString &uid, const QString &albumId) {
  const QString userId = uid.trimmed();
...
void LikesService::removeAlbumLike(const QString &uid, const QString &albumId) {
  const QString userId = uid.trimmed();
  const QString id = albumId.trimmed();
  if (userId.isEmpty() || id.isEmpty()) return;
  if (m_yandexClient == nullptr || !m_yandexClient->hasToken()) return;
...
void LikesService::addArtistLike(const QString &uid, const QString &artistId) {
  const QString userId = uid.trimmed();
...
void LikesService::removeArtistLike(const QString &uid, const QString &artistId) {
  const QString userId = uid.trimmed();
  const QString id = artistId.trimmed();
  if (userId.isEmpty() || id.isEmpty()) return;
  if (m_yandexClient == nullptr || !m_yandexClient->hasToken()) return;
...
bool LikesService::isAlbumLiked(const QString &albumId) const {
  const QString id = albumId.trimmed();
  return !id.isEmpty() && m_likedAlbumIds.contains(id);
}

bool LikesService::isArtistLiked(const QString &artistId) const {
  const QString id = artistId.trimmed();
