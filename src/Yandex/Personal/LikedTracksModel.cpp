...
LikedTracksModel::LikedTracksModel(QObject *parent) : TrackListModelBase(parent) {}

QVariant LikedTracksModel::data(const QModelIndex &index, int role) const {
...
QHash<int, QByteArray> LikedTracksModel::roleNames() const {
...
void LikedTracksModel::setTracks(const QList<Track> &tracks) {
  QList<Track> liked = tracks;

  for (int i = 0; i < liked.size(); ++i) {
    liked[i].liked = true;
  }

  TrackListModelBase::setTracks(liked);
}
...
void LikedTracksModel::setTrackLiked(const QString &trackId, bool liked) {
  TrackListModelBase::setTrackLiked(trackId, liked, LikedRole);
}

void LikedTracksModel::removeTrack(const QString &trackId) {
