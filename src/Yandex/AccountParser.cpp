...
Account AccountParser::parse(const QJsonObject &json) {
  ...
  account.serviceAvailable = accountObject.value("serviceAvailable").toBool();
  const QJsonObject plusObject = result.value("plus").toObject();
  account.hasPlus = plusObject.value("hasPlus").toBool();
  return account;
}