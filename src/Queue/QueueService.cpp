...
QueueService::QueueService(QObject *parent) : QObject(parent) {}
...
QString QueueService::sourceTitle() const {
  return m_sourceTitle;
}

QString QueueService::sourceType() const {
  return m_sourceType;
}

void QueueService::setSource(const QString &title, const QString &type) {
...
void QueueService::addTracks(const QList<Track> &tracks) {
...
void QueueService::removeTrack(int index) {
  ...
  for (int i = 0; i < m_originalTracks.size(); ++i) {
    if (m_originalTracks.at(i).id == trackId) {
      m_originalTracks.removeAt(i);
      break;
    }
  }
  ...
  } else if (index == m_currentIndex && m_currentIndex >= m_tracks.size()) {
...
void QueueService::clear() {
  const bool hadQueue = !m_tracks.isEmpty() || !m_originalTracks.isEmpty() || m_currentIndex != -1;
  m_tracks.clear();
...
bool QueueService::hasNext() const {
  return m_currentIndex >= 0 && m_currentIndex + 1 < m_tracks.size();
}
...
QueueService::RepeatMode QueueService::repeatMode() const {
  return m_repeatMode;
}
