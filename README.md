...
### v0.9 — Lyrics, Feed & Accounts 🚧

#### Lyrics (~1 день)

* [ ] Загрузка текста через `GET /tracks/{id}/supplement`
* [ ] Метод `loadSupplementary()` в TrackService
* [ ] Показ синхронизированных строк (подсветка текущей + автопрокрутка)
* [ ] Показ обычного текста без таймингов
* [ ] Кнопка 🎤 в Now Playing / панель текста в ExpandedNowPlaying

#### Feed (~2 дня)

* [ ] Бесконечная лента на главной странице
* [ ] Дозагрузка блоков лендинга (PersonalLanding уже готов)
* [ ] Пагинация / «Показать ещё»

#### Accounts (~3 дня)

* [ ] Хранение нескольких OAuth-сессий
* [ ] Переключение между аккаунтами без перезапуска
* [ ] Очистка и перезагрузка данных при смене аккаунта
