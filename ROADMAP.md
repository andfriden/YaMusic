# YaMusic Roadmap

Актуальный план развития проекта после **v0.11.0**.

---

## v0.11.0 — Выпущено

### Поиск (Full Search)

- [x] Парсинг результатов поиска по типам (треки, исполнители, альбомы, плейлисты)
- [x] Вкладки/секции на странице поиска: Треки | Альбомы | Исполнители | Плейлисты
- [x] Навигация по найденным альбомам/исполнителям/плейлистам из результатов
- [x] Вертикальная интеграция с существующими страницами (AlbumPage, ArtistPage, PlaylistPage)

### Воспроизведение

- [x] Вкладка «Похожие» в ExpandedNowPlaying
- [x] Автопереход на похожий контент при завершении очереди
- [x] Воспроизведение напрямую из стрима — offline-режим и кэш стримов убраны
- [x] Стабильный стриминг через StreamProxy QIODevice
- [x] Копирование трека «Артист — Трек» через контекстное меню во всех списках
- [x] Отправка факта прослушивания через `/play-audio` (события принимаются, но в историю не записываются — см. README)

### История прослушивания

- [x] Блок «Недавно прослушаны» читается через `/music-history`
- [x] Ynison-коннектор: регистрация устройства-плеера, «сейчас играет», heartbeat прогресса, events завершения треков

### Исправления и техдолг

- [x] Устранён warning `QIODevice::read (QSslSocket): device not open` при обрыве стрима
- [x] Моя волна: непрерывное воспроизведение через догрузку батчей
- [x] Чистка QML-варнингов
- [x] Нормализация стиля кода по проекту

---

## v0.12 — Similar Tracks (P1)

Приоритет: **средний**. Задел уже есть: `SimilarTracksModel.h` (без .cpp), API endpoint `/tracks/{id}/similar` известен.

- [x] `TrackService::loadSimilarTracks(trackId)` → GET `/tracks/{id}/similar`
- [x] `SimilarTracksModel.cpp` — реализация на базе `TrackListModelBase`
- [x] Контроллер: загрузка похожих треков при смене текущего трека
- [x] UI: вкладка «Похожие» в ExpandedNowPlaying (рядом с «Далее в очереди»)
- [x] Запуск трека из списка похожих (добавление в очередь)

---

## v0.13 — Catalog & Library Expansion (P1)

### Genre Radio Stations

- [x] Ротор/dashboard для жанров: получить станции через API ротора
- [x] UI для станций (запуск, управление)

### Album/Artist Likes — Write

- [x] Кнопка лайка альбома на AlbumPage (POST /users/{uid}/likes/albums/add-multiple)
- [x] Кнопка лайка исполнителя на ArtistPage (POST /users/{uid}/likes/artists/add-multiple)
- [x] Обновление состояния likedAlbumsModel / likedArtistsModel после операции

---

## v0.14 — Playlist Management (P2)

Приоритет: **средний**. CRUD для пользовательских плейлистов.

- [x] Создание пустого плейлиста (POST /users/{uid}/playlists/create)
- [x] Удаление плейлиста (POST /users/{uid}/playlists/{kind}/delete)
- [x] Переименование плейлиста (POST /users/{uid}/playlists/{kind}/name)
- [x] Добавление треков в плейлист (POST /users/{uid}/playlists/{kind}/change-relative)
- [x] Удаление треков из плейлиста (POST /users/{uid}/playlists/{kind}/change-relative)
- [x] UI: контекстное меню «Добавить в плейлист» на треке
- [x] UI: кнопка удаления трека при просмотре плейлиста
- [x] UI: кнопка «Добавить в плейлист» на треках в альбоме, исполнителе и плейлисте

---

## v0.10.2 — Artist Albums & Search Pagination (P2)

- [x] Все альбомы исполнителя (включая сборники): пагинация по `/artists/{id}/direct-albums`
- [x] Выбор фильтра: альбомы / синглы / сборники
- [x] Пагинация поиска (offset/page) — догрузка результатов при скролле

---

## v2.0 — Polish & Extended Features (P3)

### Synced Lyrics Highlighting

- [x] Подсветка текущей строки в полноэкранном просмотре текста в реальном времени
- [x] Автопрокрутка к текущей строке
- [x] Загрузка синхронизированного текста через `/tracks/{id}/lyrics` (LRC) с фолбэком на `/supplement`

### Podcasts

- [ ] Парсинг блоков подкастов из лендинга
- [ ] Базовая страница подкаста
- [ ] Воспроизведение подкастов (через существующий PlayerService)

### Misc

- [ ] Кеширование данных (обложки, альбомы, плейлисты)
- [ ] Offline-режим для кешированных треков
- [ ] Юнит-тесты (Google Test / Qt Test)

---

## Легенда приоритетов

| Приоритет | Описание |
|-----------|----------|
| **P0**    | Блокирующий пробел — заметен сразу, влияет на базовое впечатление |
| **P1**    | Важная функциональность — расширяет возможности, востребована |
| **P2**    | Улучшение — полезно, но не критично |
| **P3**    | Полировка и нишевые сценарии |

---

*Последнее обновление: сентябрь 2026*